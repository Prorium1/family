...
http://localhost:3000 を開き、画面上部のデモバナーで **1人目 / 2人目** の席を切り替えながら、

1. 1人目として登録（表示名と性別を選ぶ）→ 招待リンク・QR・6桁コードが自動表示
2. 招待リンクを別ウィンドウで開く → 2人目として登録すると自動でペアリング完了
   （性別の組み合わせは自由です。男性同士・女性同士でも同じ体験になります）
...
> **本番として動かすには** → [`docs/LAUNCH.md`](docs/LAUNCH.md)
> デモモードのデータはインスタンスごとの一時領域にあるため、
> 二人が別々の端末で使うには実データベース（Supabase）への切り替えが必要です。
> 手順・環境変数・検証項目はすべて手順書にまとめてあります。
