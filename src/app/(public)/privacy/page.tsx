  ...
  return (
    <article className="mx-auto max-w-2xl space-y-6 px-4 py-12 text-sm leading-relaxed">
      ...
      <p className="bg-wash rounded-card px-5 py-4">
        {appConfig.name}は、二人の最も個人的な言葉をお預かりします。
        ここに書いたことは、すべて<strong>コードとデータベースで実際に守られている</strong>ことだけです。
      ...
      <section className="space-y-2">
        ...
        <p>
          <strong>集めるもの</strong>: メールアドレス、表示名、性別（任意・「答えない」を選べます）、
          言語とタイムゾーン、あなたが書いた回答や記述、二人で保存した「私たち」の記録。
        </p>
        <p>
          <strong>集めないもの</strong>: 生年月日、住所、電話番号、勤務先、位置情報、連絡先へのアクセス、
      ...
      <section className="space-y-2">
        ...
        <p>
          ...
          これは画面上の演出ではなく、<strong>データベースの行レベルセキュリティ</strong>で強制しています。
          公開前は、たとえAPIを直接叩いても相手の回答は1件も返りません。
      ...
      <section className="space-y-2">
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li><strong>自分だけ</strong> — パートナーにもAIにも渡りません</li>
          <li><strong>AIだけ</strong> — AIは読みますが、パートナーには渡りません</li>
          <li><strong>AIの要約のみ</strong> — 原文ではなく、中立的な要約だけが相手に届きます</li>
          <li><strong>選んだ文章だけ</strong> — あなたが選んだ部分だけが共有されます</li>
          <li><strong>原文を共有</strong></li>
        </ul>
      ...
      <section className="space-y-2">
        ...
        <p>
          <strong>AIモデルの学習への利用は初期状態でオフ</strong>です。設定画面でいつでも変更できます。
        </p>
      </section>
      ...
      <section className="space-y-2">
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li><strong>Supabase</strong>（データベース・認証。保存先: 東京 / AWS）</li>
          <li><strong>Vercel</strong>（アプリの配信）</li>
          <li>
            <strong>AI提供事業者</strong>（AI分析に同意した場合のみ。
            処理のために<strong>国外へ送信される場合があります</strong>。
        ...
        <p className="rounded-card border border-border px-4 py-3">
          ...
          （AIが二人の言葉を通訳するために、サーバー側で読める必要があるためです）。
          そのうえで、<strong>通常の運用で私たちが本文を閲覧することはありません</strong>。
